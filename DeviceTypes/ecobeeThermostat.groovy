 ...
 *
 *  Tweaked by Nektarios Ioannides for a few specific things.
 *
...
metadata {
	...
	definition(name: "My Ecobee Device", author: "Yves Racine",  namespace: "yracine") {
		...
		attribute "regularScheduleName", "string"
	    attribute "regularProgramType", "string"
		attribute "authError", "string"
		attribute "climates", "string"
		command "verboseLog"
...
def verboseLog(msg) {
	sendEvent name: "verboseTrace", value: msg
}
def coolLevelUp() {
...
def heatLevelDown() {
	sendEvent name: "authError", value: 0
	int nextLevel = device.currentValue("heatingSetpoint") - 1
	def scale = getTemperatureScale()
	if (scale == 'C') {
		if (nextLevel < 10) {
...
def poll() {
	...
	sendEvent(name: 'regularScheduleName', value: currentClimate.name)
	sendEvent(name: 'regularProgramType', value: currentClimate.type)  	
...
//			sendEvent name: "verboseTrace", value: "doRequest>token= ${data.auth.access_token}"
		...
		sendEvent name: "verboseTrace", value:
			"doRequest> general or malformed request body error " + params.body + " error: " + e
	}
}
...
def getClimate(thermostatId) {
	...
	getThermostatInfo(thermostatId)
	def climatesString = '{"climates":['
	for (i in 0..data.thermostatList[0].program.climates.size() - 1) {
        def climates = data.thermostatList[0].program.climates[i]
        
        if (i != 0) {
        	climatesString += ","
        }
        
        climatesString += "{" + climates.collect { k, v -> "\"${k}\":\"${v.toString()}\"" }.join(",") + "}"

		/*if (climateName.trim().toUpperCase() == data.thermostatList[0].program.climates[
			i].name.toUpperCase()) 
		{
			foundClimate = true
			if (property == "HEATTEMP") {
				if (scale == 'C') {
					return fToC(data.thermostatList[0].program.climates[i].heatTemp)
				}
				else {
					return data.thermostatList[0].program.climates[i].heatTemp
				}
			}
		}*/
	} 
    climatesString += "]}"
	sendEvent name: "climates", value: climatesString
}

def setClimateProperty(thermostatId, climateName, property, value) {
	...
	def coolFan
	def heatFan
    log.debug "thermostatId ${thermostatId}, climateName ${climateName}, property ${property}, value ${value}"
    def valueInt = Integer.parseInt(value)
	...
	getThermostatInfo(thermostatId)
	for (i in 0..data.thermostatList[0].program.climates.size() - 1) {
		if (data.thermostatList[0].program.climates[i].name &&
            climateName.trim().toUpperCase() == data.thermostatList[0].program.climates[
			i].name.toUpperCase()) 
		{
			foundClimate = true
			
			coolTemp = data.thermostatList[0].program.climates[i].coolTemp
			heatTemp = data.thermostatList[0].program.climates[i].heatTemp
			isOptimized =  data.thermostatList[0].program.climates[i].isOptimized
			coolFan = data.thermostatList[0].program.climates[i].coolFan
			heatFan = data.thermostatList[0].program.climates[i].heatFan
	
		}
	}
	if (foundClimate) {
    	if (settings.trace) {
			log.debug "setClimateProperty> found climate to update. Update ${thermostatId}, ${climateName}, ${property} to ${value}"
			sendEvent name: "verboseTrace", value:
				"setClimateProperty> found climate to update. Update ${thermostatId}, ${climateName}, ${property} to ${value}"
		}
		if (property == "HEATTEMP") {
			if (scale == 'C') {
				heatTemp = cToF(valueInt/10)*10
			}
			else {
				heatTemp = valueInt
			}
		...
		updateClimate(thermostatId, climateName, 'false', null, coolTemp/10, heatTemp/10, isOptimized, coolFan, heatFan);
	}
    else {
			log.debug "setClimateProperty> did NOT find climate to update. Update ${thermostatId}, ${climateName}, ${property} to ${value}"
			sendEvent name: "verboseTrace", value:
				"setClimateProperty> did NOT find  climate to update. Update ${thermostatId}, ${climateName}, ${property} to ${value}"    
    }
	return null
}
...
private def refresh_tokens() {
	...
	try {
		...
		sendEvent name: "authError", value: 1
		return false
